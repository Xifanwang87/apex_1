...
class ApexBlackboard:
    ...
    prefix: str = field(default='apex:v11a:dataset')
...
class ApexUniverseBlackboard(ApexBlackboard):
    prefix: str = field(default='apex:v11a:universe_dataset')
...
def compute_financial_indicators(ds):
    ...
    ds['operating_efficiency_factor'] = crossectional_rank_scaler(ds['gross_margin']) + crossectional_rank_scaler(ds['asset_turnover'])
    ds['asset_growth'] = (ds['bs_tot_asset'] - ds['bs_tot_asset'].shift(time=252))/ds['bs_tot_asset'].shift(time=252)
    ds['earnings_revision_chg'] = (ds['eeps_nxt_yr'] - ds['eeps_nxt_yr'].shift(time=22*3))/ds['eeps_nxt_yr'].shift(time=22*3) * ds['basic_availability']
    ds['size_factor'] = crossectional_rank_scaler(ds['cur_mkt_cap'])
    ds['credit_factor'] = crossectional_rank_scaler(ds['rsk_bb_implied_cds_spread'])
