...
def compute_availability(model):
    ...
    availability = (~data.isnull()) & (data.rolling(252).median() > 10)
...
def compute_core_portfolio(model, num_core=15):
    tickers = model['universe']
    market_caps = model['market_caps']
    market_caps = market_caps.apply(fill_nas_series)
    availability = model['availability']
    selection = market_caps.rank(axis=1, ascending=False)
    ...
    core_wts = core_signal.rank(axis=1, ascending=True, pct=True).fillna(0).rolling(20).mean()
    core_wts = core_wts.divide(core_wts.abs().sum(axis=1), axis=0)[core & availability]
    core_wts[core_wts > 0.09] = 0.09 # Flattening
