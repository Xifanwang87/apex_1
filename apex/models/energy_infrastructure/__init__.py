...
class ApexUniverse:
    ...
    def __post_init__(self):
        self._db_client = redis.Redis(host='10.15.201.154', port='6379', db=20)
...
class ApexIndexUniverse:
    ...
    def subuniverses(self):
        ...
        bbg = ApexBloomberg()
        subgroups = bbg.reference(tickers, subgroup_field)[subgroup_field]
        subgroups = self.subgroups.fillna('Other')
        subgroup_count = subgroups.groupby(subgroups).count().sort_values(ascending=False)
        subgroup_count = subgroup_count[subgroup_count >= 4]
...
@dataclass
class ApexEnergyInfrastructureUniverse(ApexIndexUniverse):
    name: str = field(default='apex:universe:energy_infrastructure')
    bbg_subgroup_flds = ['INDUSTRY_SUBGROUP', 'GICS_SUB_INDUSTRY_NAME', 'BICS_LEVEL_4_SUB_INDUSTRY_NAME']
    indices: frozenset = field(default=frozenset(['AMNA Index', 'AMZ Index', 'AMEI Index', 'AMZI Index']))

    @property
    def subuniverses(self):
        _subuniverses = get_custom_mlp_subuniverses()
        subuniverses = {}
        _subuniverses['universe'] = self.tickers
        for subuniverse_name in _subuniverses:
            tickers = _subuniverses[subuniverse_name]
